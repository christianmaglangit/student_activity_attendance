...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onSwitchToSignUp }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Gamiton ang router para mu-navigate sa dashboard
      router.push('/dashboard');
    }
    // Dili na kinahanglan i-set ang loading to false diri kay mag-redirect na
  };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (insertError) {
      ...
      setLoading(false);
        ...
        // If it's enabled, the user can't log in until they verify.
        // We will log them in directly after sign up for a better user experience.
        const { error: loginError } = await supabase.auth.signInWithPassword({ email, password });
        if(loginError) {
            setMessage('Account created, but auto-login failed. Please log in manually.');
            setLoading(false);
        } else {
...
export default function HomePage() {
  ...
  return ( 
<div className="bg-gray-200 dark:bg-gray-900 min-h-screen"> <div className={`flex flex-col min-h-screen transition-all duration-300 ${isLoginOpen || isSignUpOpen ? 'blur-sm' : ''}`}> <Header onLoginClick={openLogin} onSignUpClick={openSignUp} isMenuOpen={isMenuOpen} toggleMenu={toggleMenu} /> <main className="flex-grow container mx-auto px-4 text-center flex flex-col items-center justify-center bg-gray-200"> <div className="max-w-3xl py-16"> <h2 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-gray-900 dark:text-white"> The Smart Way to Track{' '} <span className="text-green-600 dark:text-green-400"> Student Activity </span> </h2> <p className="mt-6 text-base md:text-lg text-gray-600 dark:text-gray-300"> Simplify attendance tracking, reduce administrative tasks, and gain valuable insights with our seamless, modern platform. </p> <div className="mt-10"> <p className="text-sm text-gray-500"> Click &quot;Sign Up&quot; or &quot;Login&quot; in the header to get started. </p> </div> </div> </main> <Footer /> </div> <LoginModal isOpen={isLoginOpen} onClose={() => setLoginOpen(false)} onSwitchToSignUp={openSignUp} /> <SignUpModal isOpen={isSignUpOpen} onClose={() => setSignUpOpen(false)} onSwitchToLogin={openLogin} /> </div> );
}