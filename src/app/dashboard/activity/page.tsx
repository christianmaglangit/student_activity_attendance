...
import {
  LayoutDashboard, Users, ClipboardList, QrCode, LogOut, Menu, X, UserCheck, CalendarPlus, Search, CalendarDays, Pencil, Trash2, FileText, Download
} from 'lucide-react';
...
export default function ActivityPage() {
  ...
  const openEditModal = (activity: Activity) => { setSelectedActivity(activity); setActivityName(activity.name); setStartDate(activity.start_date); setEndDate(activity.end_date); setSchedules(activity.activity_schedules); setEditModalOpen(true); };
  const openDeleteModal = (activity: Activity) => { setSelectedActivity(activity); setDeleteModalOpen(true); };
  
  const handleOpenReportModal = async (activity: Activity) => {
    ...
    if (error) { console.error("Error fetching report:", error); setReportData([]);
        ...
        data.forEach(record => {
            const studentInfo = record.students as any; if (!studentInfo) return;
        ...
        // --- BAG-O: I-sort ang data alphabetically by full_name ---
  ...
  const handleExportPDF = () => {
    if (!selectedActivity || reportData.length === 0) return;
    ...
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text(`College of Computer Studies`, pageWidth / 2, 20, { align: "center" });
    doc.text(`Computer Studies Student Organization`, pageWidth / 2, 28, { align: "center" });
    ...
    autoTable(doc, {
        head: tableHeaders,
        body: tableBody,
        startY: 58, // Adjusted Y position for the table
        theme: 'striped',
  ...
  return (
    <div className={`grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]`}>
      <div className="hidden border-r bg-white md:block dark:bg-gray-900/40 dark:border-gray-800"><SidebarContent /></div>
      <div className={`fixed inset-0 z-30 md:hidden ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setSidebarOpen(false)}></div>
      <div className={`fixed top-0 left-0 h-full w-[280px] z-40 transform transition-transform md:hidden ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}><SidebarContent /></div>
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
              <div className="relative flex-grow max-w-xs">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 border-2 bg-white border-gray-200 rounded-lg focus:outline-none focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                    suppressHydrationWarning
                  />
              </div>
