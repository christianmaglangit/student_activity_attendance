...
const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'danger' }> = ({ children, className, variant = 'primary', ...props }) => {
    const base = 'inline-flex items-center justify-center text-center px-6 h-10 font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-transform transform hover:scale-105 whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed';
    const styles = {
...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label: string }> = ({ label, id, value, disabled, ...props }) => (
    <div>
        <label htmlFor={id} className={`block mb-2 text-sm font-medium ${disabled ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'}`}>{label}</label>
        <input
            id={id}
            disabled={disabled}
            className={`uppercase w-full px-4 h-10 border-2 rounded-lg focus:outline-none focus:ring-1 transition-colors ${
                disabled 
                ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed dark:bg-gray-800 dark:border-gray-700 dark:text-gray-500' 
                : 'bg-white border-gray-200 focus:border-green-500 focus:ring-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white'
            }`}
            value={value ?? ''}
            {...props}
        />
...
export default function ActivityPage() {
    ...
    const filteredActivities = useMemo(() => {
        ...
        return activities.filter(act => act.name.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        if (startDate && endDate) {
            ...
            while (currentDate <= end) {
                ...
                newSchedules.push({
                    ...
                    am_out: ((activityType === 'whole_day' || activityType === 'half_day_am') && requireAmOut) ? (existingSchedule?.am_out ?? '12:00') : null,
                    pm_in: (activityType === 'whole_day' || activityType === 'half_day_pm') ? (existingSchedule?.pm_in ?? '13:00') : null,
                    pm_out: ((activityType === 'whole_day' || activityType === 'half_day_pm') && requirePmOut) ? (existingSchedule?.pm_out ?? '17:00') : null
                });
    ...
    }, [startDate, endDate, selectedActivity, activityType, requireAmOut, requirePmOut]);

    const handleScheduleChange = (index: number, field: keyof Omit<ActivitySchedule, 'id' | 'activity_id' | 'date'>, value: string) => {
        const updatedSchedules = [...schedules];
        updatedSchedules[index] = { ...updatedSchedules[index], [field]: value === "" ? null : value };
    ...
    const handleToggleSelectOne = (activityId: number) => {
    ...
    const handleAddActivity = async (e: React.FormEvent) => {
        ...
        if (!newActivity || !newActivity.id) {
             setMessage(`Error: Failed to retrieve the new activity ID after creation.`);
             setLoading(false);
             return;
        }
        ...
        const schedulesWithActivityId = schedules.map(sc => ({
            ...
            am_out: ((activityType === 'whole_day' || activityType === 'half_day_am') && requireAmOut) ? (sc.am_out === "" ? null : sc.am_out) : null,
            pm_in: (activityType === 'whole_day' || activityType === 'half_day_pm') ? (sc.pm_in === "" ? null : sc.pm_in) : null,
            pm_out: ((activityType === 'whole_day' || activityType === 'half_day_pm') && requirePmOut) ? (sc.pm_out === "" ? null : sc.pm_out) : null,
        }));

        if (schedulesWithActivityId.length > 0) {
            const { error: scheduleError } = await supabase.from('activity_schedules').insert(schedulesWithActivityId);
            if (scheduleError) {
    ...
    const fetchUserProfile = useCallback(async () => {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
            const { data, error } = await supabase.from('users').select('name, collegedep').eq('id', user.id).single();
    ...
    const handleUpdateActivity = async (e: React.FormEvent) => {
        ...
        await supabase.from('activity_schedules').delete().eq('activity_id', selectedActivity.id);

        if (schedules.length > 0) {
            const updatedSchedules = schedules.map(sc => {
                const finalAmIn = (activityType === 'whole_day' || activityType === 'half_day_am') ? (sc.am_in === "" ? null : sc.am_in) : null;
                const finalAmOut = ((activityType === 'whole_day' || activityType === 'half_day_am') && requireAmOut) ? (sc.am_out === "" ? null : sc.am_out) : null;
                const finalPmIn = (activityType === 'whole_day' || activityType === 'half_day_pm') ? (sc.pm_in === "" ? null : sc.pm_in) : null;
                const finalPmOut = ((activityType === 'whole_day' || activityType === 'half_day_pm') && requirePmOut) ? (sc.pm_out === "" ? null : sc.pm_out) : null;
            ...
            const { error: iErr } = await supabase.from('activity_schedules').insert(updatedSchedules);
            ...
            if (iErr) {
             ...
             setEditModalOpen(false); fetchActivities(); clearForm();
        }
        setLoading(false);
    };
    ...
    const handleDeleteActivity = async () => {
        ...
        if (error) setMessage(`Error deleting activity: ${error.message}`);
        else {
            setMessage('Activity deleted successfully!');
            setDeleteModalOpen(false); 
            setSelectedActivityIds(prev => { const next = new Set(prev); next.delete(selectedActivity.id); return next; });
            fetchActivities(); 
            clearForm();
        }
        setLoading(false);
    ...
    const handleBulkDelete = async () => {
        ...
        if (error) {
            setMessage(`Error deleting activities: ${error.message}`);
        } else {
            setMessage(`${idsToDelete.length} activities deleted successfully!`);
    ...
    const openEditModal = (activity: ActivityType) => {
        ...
        const processedSchedules = (activity.activity_schedules || []).map(s => ({
            ...s, am_in: s.am_in ?? null, am_out: s.am_out ?? null, pm_in: s.pm_in ?? null, pm_out: s.pm_out ?? null,
        }));
        ...
        setRequireAmOut(processedSchedules.length === 0 ? true : hasAmOut);
        setRequirePmOut(processedSchedules.length === 0 ? true : hasPmOut);

        setEditModalOpen(true);
        setMessage('');
    };

    const openDeleteModal = (activity: ActivityType) => { setSelectedActivity(activity); setDeleteModalOpen(true); setMessage(''); };

    const handleOpenReportModal = async (activity: ActivityType) => {
        setSelectedActivity(activity); setReportModalOpen(true); setReportLoading(true); setReportData([]); setMessage('');
        ...
        if (attendanceError) { setMessage(`Error fetching report data: ${attendanceError.message}`); setReportLoading(false); return; }
        if (!attendanceData || attendanceData.length === 0) { setMessage("No attendance records found for this activity."); setReportLoading(false); return; }

        const studentIds = [...new Set(attendanceData.map((att) => att.student_id))];
        const { data: studentsData, error: studentsError } = await supabase.from('students').select('student_id, full_name, course, year_level').in('student_id', studentIds);

        if (studentsError) { setMessage(`Error fetching student details: ${studentsError.message}`); setReportLoading(false); return; }

        const reportMap = new Map<string, ReportRow>();
        (studentsData || []).forEach(student => {
            reportMap.set(student.student_id, {
                student_id: student.student_id, full_name: student.full_name, course: student.course, year_level: student.year_level,
                'Time In (AM)': null, 'Time Out (AM)': null, 'Time In (PM)': null, 'Time Out (PM)': null, scans_completed: 0,
            });
        });
        ...
        setReportData(finalData); setReportLoading(false);
    };
    ...
    const handleExportPDF = () => {
        ...
        const requiredAmOut = selectedActivity.activity_schedules?.some(s => s.am_out !== null);
        const requiredPmOut = selectedActivity.activity_schedules?.some(s => s.pm_out !== null);

        if (showAm) { headers.push("AM In"); if (requiredAmOut) headers.push("AM Out"); }
        if (showPm) { headers.push("PM In"); if (requiredPmOut) headers.push("PM Out"); }
        headers.push("Scans");
        ...
        const tableBody = reportData.map((student, index) => {
            ...
            if (showAm) { row.push(student['Time In (AM)'] || '--'); if (requiredAmOut) row.push(student['Time Out (AM)'] || '--'); }
            if (showPm) { row.push(student['Time In (PM)'] || '--'); if (requiredPmOut) row.push(student['Time Out (PM)'] || '--'); }
            
            let expectedScans = 0;
            if (showAm) expectedScans += (requiredAmOut ? 2 : 1);
            if (showPm) expectedScans += (requiredPmOut ? 2 : 1);
            
            row.push(`${student.scans_completed} / ${expectedScans}`);
            return row;
        });

        autoTable(doc, { head: [headers], body: tableBody, startY: 58, theme: 'striped', headStyles: { fillColor: [22, 160, 133] } });
            ...
            <div className="flex flex-col h-screen overflow-hidden bg-gray-100/40 dark:bg-gray-800/40">
                ...
                <main className={`flex-1 overflow-y-auto p-4 lg:p-6 pb-20`}>
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                        ...
                        <div className="relative w-full md:flex-grow md:max-w-md flex items-center gap-2">
                            {filteredActivities.length > 0 && (
                                <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer whitespace-nowrap bg-white dark:bg-gray-700 h-10 px-3 border-2 border-gray-200 dark:border-gray-600 rounded-lg shrink-0">
                                    <input
                                        type="checkbox"
                                        checked={filteredActivities.length > 0 && selectedActivityIds.size === filteredActivities.length}
                                        onChange={handleToggleSelectAll}
                                        className="form-checkbox h-4 w-4 text-green-600 focus:ring-green-500 rounded cursor-pointer"
                                    />
                        ...
                        <div className="flex gap-2 w-full md:w-auto">
                            {selectedActivityIds.size > 0 && (
                                <Button variant="danger" onClick={() => handleSecureAction(() => setBulkDeleteModalOpen(true))} className="w-full md:w-auto flex items-center justify-center gap-2">
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                        {loading ? ( <div className="col-span-full text-center py-10"><Loader2 className="animate-spin inline mr-2" /> Loading Activities...</div> ) : filteredActivities.length > 0 ? (
                            [...filteredActivities].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map(activity => {
                                const isSelected = selectedActivityIds.has(activity.id);
                                return (
                                    <div key={activity.id} className={`bg-white p-6 rounded-lg shadow-md dark:bg-gray-900 flex flex-col relative transition-all ${isSelected ? 'ring-2 ring-green-500 bg-green-50 dark:bg-gray-800' : ''}`}>
                                                    ...
                                                    <div key={schedule.id || schedule.date} className="text-xs p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-center">
                                                            {(activity.activity_type === 'whole_day' || activity.activity_type === 'half_day_am') ? (
                                                             ...
                                                             ) : ( <> <div><span className="font-medium">AM:</span> N/A</div><div><span className="font-medium">AM:</span> N/A</div> </> )}
                                                            {(activity.activity_type === 'whole_day' || activity.activity_type === 'half_day_pm') ? (
                                                             ...
                                                             ) : ( <> <div><span className="font-medium">PM:</span> N/A</div><div><span className="font-medium">PM:</span> N/A</div> </> )}
                                                        </div>
                                                    </div>
                                                ))
                                        ...
                                        <div className="flex items-center justify-center gap-3 flex-shrink-0 mt-auto">
                        ...
                        ) : ( <div className="col-span-full text-center py-10"><p className="text-gray-500 dark:text-gray-400">{searchQuery ? 'No activities match your search.' : 'No activities created yet.'}</p></div> )}
                    </div>
                </main>
                <BottomNavBar />
            </div>
                         ...
                         <div className="flex flex-col sm:flex-row gap-4">
                             {(activityType === 'whole_day' || activityType === 'half_day_am') && (
                                 <label className="flex items-center gap-2">
                                     <input type="checkbox" checked={requireAmOut} onChange={(e) => setRequireAmOut(e.target.checked)} className="form-checkbox h-4 w-4 text-green-600 focus:ring-green-500 rounded cursor-pointer" />
                                     <span className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">Require AM Sign Out</span>
                                 </label>
                             )}
                             {(activityType === 'whole_day' || activityType === 'half_day_pm') && (
                                 <label className="flex items-center gap-2">
                                     <input type="checkbox" checked={requirePmOut} onChange={(e) => setRequirePmOut(e.target.checked)} className="form-checkbox h-4 w-4 text-green-600 focus:ring-green-500 rounded cursor-pointer" />
                                     <span className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">Require PM Sign Out</span>
                                 </label>
                             )}
                         </div>
                     </div>
                                             ...
                                             <div className={`grid grid-cols-2 ${activityType === 'whole_day' ? 'md:grid-cols-4' : 'md:grid-cols-2'} gap-4`}>
                                                 {showAmInputs && (
                                                    <>
                                                        ...
                                                        <Input label="AM Out" id={`am_out_${index}`} type="time" required={showAmInputs && requireAmOut} disabled={!requireAmOut} value={schedule.am_out ?? ''} onChange={(e) => handleScheduleChange(index, 'am_out', e.target.value)} />
                                                    </>
                                                 )}
                                                 {showPmInputs && (
                                                    <>
                                                        ...
                                                        <Input label="PM Out" id={`pm_out_${index}`} type="time" required={showPmInputs && requirePmOut} disabled={!requirePmOut} value={schedule.pm_out ?? ''} onChange={(e) => handleScheduleChange(index, 'pm_out', e.target.value)} />
                                                    </>
                                                 )}
                                             </div>
                                         </div>
            ...
            <Modal isOpen={isDeleteModalOpen} onClose={closeModal} title="Confirm Deletion" size="sm">
                <div className="text-center">
                    ...
                    <div className="flex justify-center gap-4">
                        ...
                        <Button variant="danger" onClick={handleDeleteActivity} disabled={loading}>{loading ? <><Loader2 className="animate-spin inline mr-2" /> Deleting...</> : 'Delete'}</Button>
                    </div>
                </div>
            </Modal>

            {/* Bulk Delete Confirmation Modal */}
            <Modal isOpen={isBulkDeleteModalOpen} onClose={closeModal} title="Confirm Bulk Deletion" size="sm">
                <div className="text-center">
                    <p className="mb-6">Are you sure you want to delete <span className="font-bold">{selectedActivityIds.size}</span> selected activities? This action cannot be undone.</p>
                    {message && <p className="text-red-500 text-sm mb-4">{message}</p>}
                    <div className="flex justify-center gap-4">
                        <Button variant="secondary" onClick={closeModal} disabled={loading}>Cancel</Button>
                        <Button variant="danger" onClick={handleBulkDelete} disabled={loading}>{loading ? <><Loader2 className="animate-spin inline mr-2" /> Deleting...</> : 'Delete All'}</Button>
                    </div>
                </div>
            </Modal>
                    ...
                    <>
                        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                            <div className="relative flex-grow w-full sm:max-w-sm">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                                <input type="text" placeholder="Search by ID or Year Level..." value={reportSearchQuery} onChange={(e) => setReportSearchQuery(e.target.value)} className="uppercase w-full pl-10 pr-4 h-10 border-2 bg-white border-gray-200 rounded-lg focus:outline-none focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" suppressHydrationWarning />
                            </div>
                            <Button onClick={handleExportPDF} disabled={reportData.length === 0} className="w-full sm:w-auto flex items-center justify-center gap-2"><Download size={18} className="inline" /> Export PDF</Button>
                        </div>
                                ...
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {filteredReportData.map((student, index) => {
                                            const reqAmOut = selectedActivity?.activity_schedules?.some(s => s.am_out !== null);
                                            const reqPmOut = selectedActivity?.activity_schedules?.some(s => s.pm_out !== null);
                                            let totalScans = 0;
                                            if (selectedActivity?.activity_type === 'whole_day' || selectedActivity?.activity_type === 'half_day_am') totalScans += reqAmOut ? 2 : 1;
                                            if (selectedActivity?.activity_type === 'whole_day' || selectedActivity?.activity_type === 'half_day_pm') totalScans += reqPmOut ? 2 : 1;
                        ...
                        ) : ( <div className="text-center py-10"><p className="text-gray-500 dark:text-gray-400">{reportSearchQuery ? 'No students match the search.' : 'No attendance data available for this activity yet.'}</p></div> )}
                    </>
                )}
            </Modal>

            {isPasswordModalOpen && (
                <PasswordConfirmationModal onConfirm={handleConfirmPassword} onClose={() => setIsPasswordModalOpen(false)} loading={isVerifyingPassword} error={passwordError} />
            )}
        </div>
    );
}