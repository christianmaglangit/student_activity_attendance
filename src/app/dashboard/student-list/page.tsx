...
import {
  LayoutDashboard, Users, ClipboardList, QrCode, LogOut, Menu, X, UserCheck, Download, UserPlus, Edit, Trash2, Search
} from 'lucide-react';
...
// A more specific type for our form data, excluding server-generated fields
...
export default function StudentListPage() {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      closeModal();
      setModalState({ type: 'qr', student: data });
      fetchStudents();
    }
    setLoading(false);
  };
  ...
  const handleUpdateStudent = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from('students')
      .update(formData)
      .eq('student_id', modalState.student.student_id);
  ...
  const handleDownloadQr = useCallback(() => {
    if (!qrCodeRef.current || !modalState.student) return;
    toPng(qrCodeRef.current, { cacheBust: true })
      .then((dataUrl) => {
        const link = document.createElement('a');
        link.download = `${modalState.student!.full_name}_${modalState.student!.student_id}.png`;
        link.href = dataUrl;
        link.click();
      })
  ...
  return (
    <div className={`grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]`}>
      ...
      <div className="flex flex-col">
        ...
        <main
          className={`flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6 bg-gray-100/40 dark:bg-gray-800/40 transition-filter duration-300 ${
            isModalActive ? 'blur-sm' : ''
          }`}
        >
                  ...
                  {loading ? (
                      ...
                      <tr key={student.student_id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => openQrModal(student)} className="p-2 text-gray-500 hover:text-black"><QrCode size={18} /></button>
                    ...
                    <tr>
                      <td colSpan={5} className="text-center p-6 text-gray-500">
                        {searchQuery ? 'No students match your search.' : 'No students found. Add a new one to get started.'}
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {/* --- Modals --- */}
      <Modal isOpen={modalState.type === 'add'} onClose={closeModal} title="Add New Student">
        <form onSubmit={handleAddStudent} className="space-y-4">
          <Input label="Full Name" id="full_name" type="text" required value={formData.full_name} onChange={handleFormChange} />
          <Input label="ID Number" id="student_id" type="text" required value={formData.student_id} onChange={handleFormChange} />
          ...
          <Button type="submit" className="w-full mt-2" disabled={loading}>{loading ? 'Saving...' : 'Save Student'}</Button>
        </form>
      </Modal>
      
      <Modal isOpen={modalState.type === 'edit'} onClose={closeModal} title="Edit Student">
        <form onSubmit={handleUpdateStudent} className="space-y-4">
            ...
            <Input label="Course" id="course" type="text" required value={formData.course} onChange={handleFormChange} />
            {message && <p className={`text-center text-sm ${message.startsWith('Error') ? 'text-red-500' : 'text-green-600'}`}>{message}</p>}
            <Button type="submit" className="w-full mt-2" disabled={loading}>{loading ? 'Updating...' : 'Update Student'}</Button>
        </form>
      </Modal>
          ...
          <Modal isOpen={modalState.type === 'qr'} onClose={closeModal} title="Student QR Code" size="sm">
            <div className="flex flex-col items-center gap-4">
              <div ref={qrCodeRef} className="bg-white p-6 rounded-lg text-center">
                {/* ✅ UPDATED QR CODE VALUE */}
                <QRCodeCanvas
                  value={JSON.stringify({
                    student_id: modalState.student.student_id,
                    full_name: modalState.student.full_name,
                    course: modalState.student.course,
                    year_level: modalState.student.year_level,
                  })}
                  size={200}
                  className="mx-auto"
                />
              ...
              <Button onClick={handleDownloadQr} className="w-auto flex items-center gap-2">
          ...
          <Modal isOpen={modalState.type === 'delete'} onClose={closeModal} title="Confirm Deletion" size="sm">
            <div className="text-center">
              ...
              <div className="flex gap-4 mt-6">
                ...
                <Button variant="danger" className="w-full" onClick={handleDeleteStudent} disabled={loading}>
